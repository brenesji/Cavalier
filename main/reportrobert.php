...
<form action="reportrobert.php" method="get">
<center><strong>Desde : <input type="text" style="width: 223px; padding:14px;" name="d1" class="tcal" value="" /> Hasta: <input type="text" style="width: 223px; padding:14px;" name="d2" class="tcal" value="" />
 ...
 <button class="btn btn-info" style="width: 123px; height:35px; margin-top:-8px;margin-left:8px;" type="submit"><i class="icon icon-search icon-large"></i> Robert</button>
...
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: left;">
	...
	<tbody>
			...
			<?php
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$result = $db->prepare("SELECT * FROM sales_order WHERE gen_name = 'Nancy Brenes' AND date BETWEEN :a AND :b ORDER by transaction_id DESC ");
	...
	<thead>
		<tr>
			...
			<?php
				...
				$results = $db->prepare("SELECT sum(amount) FROM sales_order WHERE gen_name = 'Nancy Brenes' AND date BETWEEN :a AND :b ");
				...
				<th colspan="1" style="border-top:1px solid #999999">
			<?php 
				$resultia = $db->prepare("SELECT sum(profit) FROM sales_order WHERE gen_name = 'Nancy Brenes' AND date BETWEEN :c AND :d");
