...
function createRandomPassword() {
	...
	$i = 0;
	$pass = '' ;
	while ($i <= 10) {  //hay que cambiar todos los modulos para generar este ramdom
...
<?php } ?>				
	...
	<div class="span10">
		<div class="contentheader">
			...
			<ul class="breadcrumb">
			<a href="index.php"><li>Modulo Principal</li></a> /
...
<option></option>
	...
	?>
		<option value="<?php echo $row['product_id'];?>"><?php echo $row['product_code']; ?> - <?php echo $row['supplier']; ?> </option>
	<?php
				}
			?>
</select>
...
<input type="hidden" name="date" value="<?php date_default_timezone_set("America/Costa_Rica"); echo date("m/d/y"); ?>" />
...
<table class="table table-bordered" id="resultTable" data-responsive="table">
	...
	<tbody>
			...
			<?php
				$id=$_GET['invoice'];
				include('../connect.php');
				$result1 = $db->prepare("SELECT * FROM sales WHERE invoice= :userid");
				$result1->bindParam(':userid', $id);
				$result1->execute();
				for($i=1; $row = $result1->fetch(); $i++){
					
					$barber=$row['supplier'];
					$compra=$row['product_code'];
				}
				/*hice este metodo para que guarde la varieble barbero aqui, luego la almacene en $getbarber y eso lo paso a checkout para que el haga la factura*/
			?>
...
<a rel="facebox" href="checkout.php?pt=
<?php echo $_GET['id']?>
&invoice=<?php echo $_GET['invoice']?>
&total=<?php echo $fgfg ?>
&totalprof=<?php echo $asd ?> 
&getbarber=<?php echo $barber ?>  
&getproduct=<?php echo $compra ?>
&cashier=<?php echo $_SESSION['SESS_FIRST_NAME']?>"><button class="btn btn-success btn-large btn-block"><i class="icon icon-save icon-large"></i> GUARDAR</button></a>
